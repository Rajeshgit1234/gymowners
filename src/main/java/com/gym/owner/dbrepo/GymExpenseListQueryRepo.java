...
public interface GymExpenseListQueryRepo extends JpaRepository<GymExpenseListQuery, Integer> {
    ...
    @Query(
            value = "SELECT list.id,owner.name,list.created_on,list.amount ,list.exp_id,master.expense_item,list.exp_remarks FROM gym_expense_list list,gym_owners  owner,expense_master  master WHERE  list.status=true and list.gym_id in (:gym)  and list.exp_id=master.id and list.created_by=owner.id group by list.created_by ,list.id,owner.name,list.created_on,list.amount ,list.exp_id,master.expense_item,list.exp_remarks  order by list.created_by desc LIMIT :limit OFFSET :offset",
            nativeQuery = true
    )
